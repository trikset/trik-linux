...
int init_module(void)
{
	...
	ret = gpio_request_one(trik_jd1_pdata.gpio_d1a, 
									GPIOF_OUT_INIT_LOW|GPIOF_EXPORT_DIR_FIXED|GPIOF_EXPORT,
									"trik_jd1->gpio_d1a");
	if(ret){
		pr_err("%s: trik_jd1->gpio_d1a failed\n",__func__);
		goto exit_gp_rq_d1a;
	}
	ret = gpio_request(trik_jd1_pdata.gpio_d1b,
									"trik_jd1->gpio_d1b");
	if(ret){
		pr_err("%s: gtrik_jd1->gpio_d1b failed\n",__func__);
		goto exit_gp_rq_d1b;
	}
	ret = gpio_request_one(trik_jd1_pdata.gpio_d1e,
									GPIOF_OUT_INIT_HIGH|GPIOF_EXPORT_DIR_FIXED|GPIOF_EXPORT, 
									"trik_jd1->gpio_d1e");
	if(ret){
		pr_err("%s: trik_jd1->gpio_d1e failed\n",__func__);
		goto exit_gp_rq_d1e;
	}
...
exit_gp_rq_d1e:
	gpio_free(trik_jd1_pdata.gpio_d1b);
exit_gp_rq_d1b:
	gpio_free(trik_jd1_pdata.gpio_d1a);
exit_gp_rq_d1a:
